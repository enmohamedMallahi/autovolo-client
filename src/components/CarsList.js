...
const CarsList = ({ cars, openModal, showBookButton, loading }) => {
  if (loading) {
    return <CarsListSkeleton />;
  }
    ...
    <ul className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 py-4 md:px-8 px-4">
      {cars.map((car) => (
        <li key={car.id} className="bg-white rounded-md shadow-md flex flex-col items-center">
          <img src={car.images[0]} alt={car.model} className="w-full h-28 md:h-32 lg:h-40 object-cover rounded-md" />
          ...
          {showBookButton && (
            <button
              ...
              className="bg-blue-500 hover:bg-blue-700  text-white w-full p-2 rounded-md transition duration-150"
            >
              Book Now &gt;
            </button>
          )}
